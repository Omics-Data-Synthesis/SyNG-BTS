...
# -*- coding: utf-8 -*-
"""
五癌症亚类增强完整工作流 (SyNG-BTS 2.6)
- 5 种癌症亚类: COAD, LAML, PAAD, READ, SKCM
- 3 种标准化: raw / TC / DESeq
- 3 个 CVAE 模型: CVAE1-10, CVAE1-50, CVAE1-100
- 1 种 data_type: miRNA
- 20 个 batch
总实验数: 5 × 3 × 3 × 1 × 20 = 900 次
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(_LOG_DIR / "Complete_workflow_FiveSubtypes.log", encoding="utf-8"),
        logging.StreamHandler(),
    ],
)
logger = logging.getLogger(__name__)

# 5 种癌症亚类: (数据目录名, 文件名前缀，如 COAD -> COADPositive_5-2)
CANCER_SUBTYPES = [
    ("COAD_5-2", "COAD"),
    ("LAML_5-2", "LAML"),
    ("PAAD_5-2", "PAAD"),
...
MODELS = ["CVAE1-10", "CVAE1-50", "CVAE1-100"]
DATA_TYPES = ["miRNA"]
N_BATCHES = 20
...
def create_run_output_dir(base_path, case_id, normalization_method, batch_num, model_name, data_type):
    """创建单次实验输出目录: .../case_id/norm/batch_k/model_name/data_type"""
    output_path = Path(base_path) / case_id / normalization_method / f"batch_{batch_num}" / model_name / data_type
...
def write_experiment_record(
    ...
    line = (
        f"{case_id}-{normalization_method}-batch{batch_num}-{model_name}-{data_type}-{status}"
...
def run_syng_bts_experiment(
    ...
    logger.info(f"开始: {case_id} | model={model_name} data_type={data_type}")
    ...
    try:
        ...
        logger.info(f"完成: {case_id} {model_name} / {data_type}")
        ...
        logger.error(f"失败: {case_id} {model_name} / {data_type}, 错误: {e}")
        raise
...
def process_single_dataset(
    ...
    """处理一种 (癌症×标准化) 下的所有 batch 与 (model × data_type) 组合。"""
    try:
        ...
        for model_name in models:
            for data_type in data_types:
                output_dir = create_run_output_dir(
                    base_output_path, case_id, normalization_method,
                    batch_num, model_name, data_type
                )
...
def main():
    ...
    logger.info("五癌症亚类增强工作流开始 (SyNG-BTS 2.6)")

    data_case_root = _REPO_ROOT / "syng_bts" / "data" / "case"
    output_base_path = _REPO_ROOT / "syng_bts" / "data" / "case" / "Augmentation-FiveSubtypes-2026-02-24"
    output_base_path.mkdir(parents=True, exist_ok=True)

    record_file = _LOG_DIR / "Complete_workflow_FiveSubtypes_records.txt"
    with open(record_file, "w", encoding="utf-8") as f:
        f.write("五癌症亚类增强实验记录 (SyNG-BTS 2.6)\n")
        f.write("格式: case_id-标准化-batch-模型-data_type-状态|时间戳|耗时|完成比例|已完成/总数\n")
        f.write("=" * 120 + "\n")
    ...
    logger.info(f"总实验数: {total_experiments} (5×3×20×3×1)")
    logger.info(
        f"癌症: {[c[0] for c in CANCER_SUBTYPES]}, "
        f"标准化: {NORMALIZATION_METHODS}, 模型: {MODELS}, data_type: {DATA_TYPES}, batches: {N_BATCHES}"
    )
