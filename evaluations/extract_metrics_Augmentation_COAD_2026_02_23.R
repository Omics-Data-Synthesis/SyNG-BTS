...
# 文件名前缀（与增强数据文件名一致）
cancer_prefix <- "COAD_5-2"

# 三种标准化：与目录 Augmentation-COAD-2026-02-23 下的 raw / TC / DESeq 一致
...
# 根目录：指向这轮增强数据
base_path <- "/Users/yanjiechen/Documents/Github/SyNG-BTS_2.6/syng_bts/data/case/Augmentation-COAD-2026-02-23"
...
## 主循环：遍历 norm / batch / model / data_type -------------------------
...
for (norm in norm_list) {
    ...
    if (length(model_list) == 0) {
        warning("norm = ", norm, " 下未探测到模型子目录，跳过。")
        next
    }
    cat("norm = ", norm, " 将评估的模型: ", paste(model_list, collapse = ", "), "\n", sep = "")
    ...
    for (batch in batch_list) {
        ...
        cat("发现 batch", batch, "的真实数据文件:", real_file, "\n")
        ...
        for (model in model_list) {
            for (data_type in data_type_list) {
                # 生成数据所在目录：DESeq/batch_k/<model>/<data_type>/
                ...
                cat("Processing:",
                    basename(real_file), "+",
                    basename(gen_file), " (model =", model, ", data_type =", data_type, ")\n"
                )
...
output_dir <- "/Users/yanjiechen/Documents/Github/SyNG-BTS_2.6/evaluations/Augmentation-2026-02-23"
if (!dir.exists(output_dir)) {
    dir.create(output_dir, recursive = TRUE)
}

output_file <- file.path(output_dir, "COAD_Augmentation-2026-02-23_evaluation_results.csv")
