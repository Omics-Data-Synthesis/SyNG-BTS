...
# -*- coding: utf-8 -*-
"""
COAD-LAML 增强完整工作流 (SyNG-BTS 2.6)
- 2 种癌症亚类: COAD, LAML
- 3 种标准化: raw / TC / DESeq
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(_LOG_DIR / "Complete_workflow_COAD-LAML.log", encoding="utf-8"),
        logging.StreamHandler(),
    ],
)
logger = logging.getLogger(__name__)

# 2 种癌症亚类: (数据目录名, 文件名前缀，如 COAD -> COADPositive_5-2)
CANCER_SUBTYPES = [
    ("COAD_5-2", "COAD"),
    ("LAML_5-2", "LAML"),
]
...
def save_split_data(train_data, test_data, batch_dir, base_filename):
    """将 train/test 保存到 batch 目录（该 batch 下所有配置共用）。"""
...
def create_run_output_dir(
    ...
    """创建单次实验输出目录: .../case_id/norm/batch_k/offaug_xxx/model_name/data_type"""
...
def process_single_dataset(
    ...
    """处理一种 (癌症×标准化) 下的所有 batch 与 (off_aug × model × data_type) 组合。"""
    try:
...
def main():
    ...
    logger.info("COAD-LAML 增强工作流开始 (SyNG-BTS 2.6)")
    ...
    record_file = _LOG_DIR / "Complete_workflow_COAD-LAML_records.txt"
    with open(record_file, "w", encoding="utf-8") as f:
        f.write("COAD-LAML 增强实验记录 (SyNG-BTS 2.6)\n")
    ...
    logger.info(f"总实验数: {total_experiments} (2×3×20×2×3×1)")
    logger.info(
